...
const AppliedJobsList = () => {
  ...
  if (loading) return <p className="text-center text-xl font-semibold">Loading applied jobs...</p>;
  if (error) return <p className="text-center text-red-500 text-lg">{error}</p>;

  return (
    <div className="max-w-5xl mx-auto p-8 bg-white shadow-lg rounded-lg border border-gray-200 animate-fadeIn">
      <h2 className="text-3xl font-bold mb-6 text-gray-800 text-center">My Applied Jobs</h2>
      ...
      <div className="flex justify-between mb-6">
        ...
        >
          ...
          <option value="job_employer">Sort by Employer</option>
        </select>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {jobs.map((job) => (
          <div key={job.id} className="p-6 border rounded-lg shadow-lg bg-gray-50 hover:shadow-xl transition-transform transform hover:scale-105">
            <h3 className="text-2xl font-bold text-gray-900">{job.job_title}</h3>
            <p className="text-md text-gray-600">{job.job_location} | {job.job_employer}</p>
            <p className="text-sm text-gray-500 mt-2">Applied on: {new Date(job.applied_at).toLocaleDateString()}</p>
            <a
              href={job.resume}
